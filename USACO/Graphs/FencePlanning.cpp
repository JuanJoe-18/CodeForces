...
struct DSU {
   ...
   * @param n Cantidad de elementos (0-indexed o 1-indexed, usar tamaño n+1 si
   * es 1-indexed).
   */
...
template <typename T = ll> struct Graph {
   ...
   *         y el segundo elemento es un vector con los índices de las aristas
   * del MST.
   */
...
struct TwoSat {
  ...
  // Agregar implicación: Si U pasa, entonces V tiene que pasar obligatoriamente
  // (U => V) Es lógicamente equivalente a (!U OR V)
...
ll perim(std::vector<int> c) {
  if (c.empty())
    return 0;
  ll min_x = X[c[0]], max_x = X[c[0]];
  ll min_y = Y[c[0]], max_y = Y[c[0]];
