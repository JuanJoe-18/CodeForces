...
template <typename T> T dot(Point<T> a, Point<T> b) {
...
template <typename T> T cross(Point<T> p, Point<T> a, Point<T> b) {
  return cross(a - p, b - p);
}

template <typename T> T norm2(Point<T> p) { return dot(p, p); }

template <typename T> long double norm(Point<T> p) { return sqrtl(norm2(p)); }
...
bool segment_intersect(Point<T> a, Point<T> b, Point<T> c, Point<T> d) {
  ...
  if (o1 == 0 && on_segment(c, a, b))
    return true;
  if (o2 == 0 && on_segment(d, a, b))
    return true;
  if (o3 == 0 && on_segment(a, c, d))
    return true;
  if (o4 == 0 && on_segment(b, c, d))
    return true;
...
int point_in_polygon(const vector<Point<T>> &poly, Point<T> p) {
  ...
  for (int i = 0; i < n; i++) {
    ...
    if (a.y > b.y)
      swap(a, b);
    if (p.y >= a.y && p.y < b.y && cross(p, a, b) > 0)
      hits++;
  }
  return (hits % 2 != 0) ? 2 : 0;
}
...
double circle_triangle_area(Point<double> a, Point<double> b, double r) {
  ...
  auto dot = [](Point<double> p1, Point<double> p2) {
...
double circle_polygon_area(const vector<Point<double>> &poly, Point<double> c,
                           double r) {
  double ans = 0;
  int n = poly.size();
  for (int i = 0; i < n; i++) {
