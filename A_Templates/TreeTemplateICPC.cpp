...
// 2. Binary Lifting / LCA + Path Queries (Dist, K-ésimo ancestro, Min/Max/Sum en caminos)
// 3. Euler Tour / Subtree flattening (Subtree updates & queries)
// 4. Heavy-Light Decomposition (HLD) con Segment Tree (Path & Subtree updates/queries)
...
struct Tree {
    ...
    // Tree Distances I: Distancia máxima desde cada nodo hacia cualquier otro O(N)
...
// ====================================================================
// Permite actualizaciones y consultas en caminos y subárboles en O(log^2 N) o O(log N).
template <typename T = ll>
struct HLD {
    int n;
    ...
    void update_node(int u, T val) {
        seg.update(1, 1, n, pos[u], pos[u], val);
    }
...
// Ideal para contar caminos con propiedad K, distancias mínimas a nodos marcados, etc.
...
// ====================================================================
// Comprueba si dos árboles o subárboles tienen la misma estructura sin importar etiquetado.
struct TreeHashing {
    ...
    static uint64_t get_rooted_hash(int u, int p, const vector<vector<int>>& adj) {
        uint64_t h = 1;
    ...
    static bool are_isomorphic(int n1, const vector<vector<int>>& adj1, 
                              int n2, const vector<vector<int>>& adj2) {
        ...
        for (int root2 : c2) {
            if (h1 == get_rooted_hash(root2, 0, adj2)) return true;
        }
        if (c1.size() > 1) {
            uint64_t h1_alt = get_rooted_hash(c1[1], 0, adj1);
            for (int root2 : c2) {
                if (h1_alt == get_rooted_hash(root2, 0, adj2)) return true;
            }
        }
        return false;
    }
...
// Ejemplo implementado: Suma de distancias a todos los demás nodos (Tree Distances II).
struct RerootingDP {
    int n;
    vector<vector<int>> adj;
    ...
    void dfs_reroot(int u, int p = 0) {
        for (int v : adj[u]) {
            if (v != p) {
                ...
                // El resto de los nodos (n - sz[v]) están 1 paso más lejos (+ (n - sz[v]))
...
// ====================================================================
// MAIN DE PRUEBA Y DEMOSTRACIÓN DE USO
// ====================================================================
int main() {
    ...
    // }
    // T.init(1);
    // BinaryLifting<ll> bl(T, 1);
    // cout << "LCA(2, 3): " << bl.lca(2, 3) << "\n";
    // cout << "Distancia(2, 3): " << bl.dist_weighted(2, 3) << "\n";
