...
export default function Planner() {
  ...
  const weekStats = useMemo(() => {
    return WEEKS.map((wk, wi) => {
      let total = 0, done = 0, mins = 0, starred = 0, starredDone = 0;
      ...
      return { total, done, mins, starred, starredDone };
    });
  }, [checked]);

  const globalStats = useMemo(() => {
    let total=0, done=0, cses=0, usaco=0, cf=0, starred=0, starredDone=0;
    WEEKS.forEach((wk,wi) => wk.days.forEach((d,di) => d[1].forEach((p,pi) => {
      total++;
      if(checked[`${wi}-${di}-${pi}`]) done++;
      if(p[1]==="C") cses++;
      else if(p[1].startsWith("U")) usaco++;
      else if(p[1]==="CF") cf++;
      if(p[5]) { starred++; if(checked[`${wi}-${di}-${pi}`]) starredDone++; }
    })));
    return {total,done,cses,usaco,cf,starred,starredDone};
  }, [checked]);
  ...
  const techStats = useMemo(() => {
    ...
    WEEKS.forEach((wk,wi) => wk.days.forEach((d,di) => d[1].forEach((p,pi) => {
      ...
      stats[tech].total++;
      if (checked[`${wi}-${di}-${pi}`]) stats[tech].done++;
    })));
    return stats;
  }, [checked]);
              ...
              <div key={di} style={{
                ...
                <div style={{display:"flex",flexDirection:"column",gap:5,paddingLeft:4}}>
                  {filteredProblems.map(({p, i: pi}) => {
                    ...
                    const isDone = checked[`${weekIdx}-${di}-${pi}`];
                    ...
                    return (
                      <div key={pi}
                        ...
                        style={{
                          ...
                          opacity:isDone?0.4:1, transition:"opacity 0.15s",
                        }}>
                        <div style={{
                          width:16,height:16,borderRadius:4,flexShrink:0,
                          border:isDone?`2px solid #00e5a0`:"2px solid rgba(255,255,255,0.15)",
                          background:isDone?"#00e5a0":"transparent",
                          display:"flex",alignItems:"center",justifyContent:"center",
                          transition:"all 0.15s",
                        }}>
                          {isDone && <span style={{fontSize:10,color:"#07070c",fontWeight:800}}>✓</span>}
                        </div>
